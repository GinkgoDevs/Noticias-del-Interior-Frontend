...
export function Crossword() {
    ...
    const fetchTodayCrossword = async () => {
        try {
            setLoading(true)
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/games/crossword/today`)
    ...
    const handleCellChange = (r: number, c: number, val: string) => {
        ...
        if (newVal.length > 1) {
            const oldVal = userGrid[r][c].toUpperCase();
    ...
    const checkWin = (grid: string[][]) => {
        ...
        for (let r = 0; r < data.size; r++) {
            for (let c = 0; c < data.size; c++) {
                if (data.solution[r][c] !== '' && grid[r][c] !== data.solution[r][c]) return
            }
        }
        setSolved(true)
    }
    ...
    const getHint = () => {
        ...
        for (let r = 0; r < data.size; r++) {
            for (let c = 0; c < data.size; c++) {
                if (data.solution[r][c] !== '' && userGrid[r][c] !== data.solution[r][c]) pending.push({ r, c })
            }
        }
        if (pending.length > 0) {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 grid grid-cols-1 gap-6">
                <Card className="shadow-lg border-2">
                    <CardHeader className="bg-muted/50 dark:bg-muted/20 py-3 border-b">
                    ...
                    <CardContent className="p-0">
                        <div className="max-h-[300px] lg:max-h-[400px] overflow-y-auto divide-y divide-border/50">
                            {acrossClues.map((w, idx) => (
                                <div
                                    key={w.id}
                                    className={cn(
                                        "p-3 text-sm transition-colors cursor-pointer hover:bg-accent",
                                        selectedCell && data.words.find(word => word.id === w.id && word.row === selectedCell.r && word.col <= selectedCell.c && word.col + word.word.length > selectedCell.c && word.direction === 'ACROSS') ? "bg-blue-500/10" : ""
                                    )}
                        ...
                        <div className="max-h-[300px] lg:max-h-[400px] overflow-y-auto divide-y divide-border/50">
                            {downClues.map((w, idx) => (
                                <div
                                    ...
                                    className={cn(
                                        "p-3 text-sm transition-colors cursor-pointer hover:bg-accent",
                                        selectedCell && data.words.find(word => word.id === w.id && word.col === selectedCell.c && word.row <= selectedCell.r && word.row + word.word.length > selectedCell.r && word.direction === 'DOWN') ? "bg-purple-500/10" : ""
                                    )}
                                    onClick={() => cellsRef.current[w.row][w.col]?.focus()}
                                >
