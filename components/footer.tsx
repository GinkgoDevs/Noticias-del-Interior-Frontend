"use client"

import Link from "next/link"
import Image from "next/image"

export function Footer() {
    return (
        <footer className="bg-secondary text-secondary-foreground mt-20 pt-16 pb-8 border-t border-primary/20">
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mb-12">
                    {/* Brand Column */}
                    <div>
                        <Link href="/" className="flex items-center mb-6">
                            <Image
                                src="/logo-claro.png"
                                alt="Noticias del Interior"
                                width={280}
                                height={70}
                                className="w-[160px] md:w-[220px] h-auto dark:hidden"
                            />
                            <Image
                                src="/logo-oscuro.png"
                                alt="Noticias del Interior"
                                width={280}
                                height={70}
                                className="w-[160px] md:w-[220px] h-auto hidden dark:block"
                            />
                        </Link>
                        <p className="text-secondary-foreground/60 text-sm leading-relaxed mb-6 font-light">
                            Desde 1992 conectando a todo el país con información veraz, plural e independiente. Somos la voz de las provincias.
                        </p>
                        <div className="flex space-x-4">
                            <SocialLink icon="facebook" />
                            <SocialLink icon="x" />
                            <SocialLink icon="instagram" />
                        </div>
                    </div>

                    {/* Sections */}
                    <div>
                        <h4 className="font-bold mb-6 uppercase text-sm tracking-widest border-b border-primary/20 pb-2">Secciones</h4>
                        <ul className="space-y-3 text-secondary-foreground/70 text-sm font-light">
                            <li><Link href="/economia" className="hover:text-primary transition-colors">Economía & Negocios</Link></li>
                            <li><Link href="/politica" className="hover:text-primary transition-colors">Política & Estado</Link></li>
                            <li><Link href="/policiales" className="hover:text-primary transition-colors">Sociedad & Salud</Link></li>
                            <li><Link href="/deportes" className="hover:text-primary transition-colors">Deportes & Pasión</Link></li>
                            <li><Link href="/espectaculo" className="hover:text-primary transition-colors">Cultura & Espectáculos</Link></li>
                        </ul>
                    </div>

                    {/* Institutional */}
                    <div>
                        <h4 className="font-bold mb-6 uppercase text-sm tracking-widest border-b border-primary/20 pb-2">Institucional</h4>
                        <ul className="space-y-3 text-secondary-foreground/70 text-sm font-light">
                            <li><Link href="/contacto" className="hover:text-primary transition-colors">Contacto</Link></li>
                            <li><Link href="/terminos" className="hover:text-primary transition-colors">Términos y Condiciones</Link></li>
                            <li><Link href="/anunciar" className="hover:text-primary transition-colors">Anunciar con nosotros</Link></li>
                        </ul>
                    </div>
                </div>

                <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center text-xs text-secondary-foreground/50">
                    <p>© {new Date().getFullYear()} Editorial El Interior S.A. Todos los derechos reservados.</p>
                    <div className="mt-4 md:mt-0 flex space-x-6">
                        <span>Diseñado con <span className="text-primary font-bold">♥</span> desde Argentina</span>
                    </div>
                </div>
            </div>
        </footer>
    )
}

function SocialLink({ icon }: { icon: string }) {
    const icons: Record<string, React.ReactNode> = {
        facebook: (
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
            </svg>
        ),
        x: <span className="font-bold text-sm">X</span>,
        instagram: (
            <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
            </svg>
        ),
    }

    return (
        <Link
            href="#"
            target="_blank"
            className="w-10 h-10 rounded-full bg-background flex items-center justify-center hover:bg-primary transition-all text-secondary-foreground hover:text-white border border-white/5"
        >
            {icons[icon]}
        </Link>
    )
}
